...
class MRMLUtility(object):
    ...
    def loadMRMLNode(node_name, file_dir, file_name, file_type):
        ...
        if node is None:
            ...
            node = slicer.util.loadNodeFromFile(file_path, file_type, properties)
            # node = node[1]
            if node is None:
    ...
    def saveMRMLNode(node, case_dir):
        if slicer.mrmlScene.GetFirstNodeByName(node.GetName()):
    ...
    def removeMRMLNode(node):
        if node == -1:
            logging.error('Deleting non-existent node')
            return
        if slicer.mrmlScene.GetFirstNodeByName(node.GetName()):
